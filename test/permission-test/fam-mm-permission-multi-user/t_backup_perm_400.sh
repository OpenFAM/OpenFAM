...
# As user1:user1 Create a region with permission 777 of size 100B
su -c "./fam_create_region myRegion 100 777" user1 || test_failed "Create Region failed: $?" 1

# As user1 : Allocate DataItem
su -c "./fam_allocate_data myRegion myData 50 400" user1 2>&1 1>/dev/null || test_failed "Allocate DataItem user1 failed: $?"
...
! su -c "./fam_delete_backup_data myBackup" user1 2>&1 1>/dev/null || test_failed "Delete myBackup user1 expected to fail: $?"
