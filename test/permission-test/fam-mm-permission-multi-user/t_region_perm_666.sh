...
su -c "./fam_create_region myRegion  100 666" user1 || test_failed "Create Region failed: $?" 1
...
#Testcases which fails. Need to be addressed.
...
su -c "./fam_allocate_data myRegion myData 50 666" user4 2>&1 1>/dev/null 2>/dev/null || test_failed "Allocate DataItem user4 failed: $?"
