...
su -c "./fam_restore_backup_data myRegion myData myBackup" user2 2>&1 1>/dev/null || test_failed "Restore DataItem user2 failed: $?"


# As user2 : Delete backup
 su -c "./fam_delete_backup_data myBackup" user2 2>&1 1>/dev/null || test_failed "Delete myBackup user2 failed: $?"
...
# As user3 : Restore to DataItem
 su -c "./fam_restore_backup_data myRegion myData myBackup" user3  || test_failed "Restore DataItem user3 failed: $?"
# As user3 : Delete backup
 su -c "./fam_delete_backup_data myBackup" user3 || test_failed "Delete myBackup user3 failed: $?"

# As user4 : Backup DataItem
 su -c "./fam_backup_data myRegion myData myBackup" user4  2>&1 1>/dev/null  || test_failed "Backup DataItem user4 failed: $?"

# As user4 : Restore to DataItem
 su -c "./fam_restore_backup_data myRegion myData myBackup" user4   2>&1 1>/dev/null || test_failed "Restore DataItem user4 failed: $?"

# As user4 : Delete backup
 su -c "./fam_delete_backup_data myBackup" user4 2>&1 1>/dev/null || test_failed "Delete myBackup user4 failed: $?"
