...
# As user1:user1 Create a region with permission 460 of size 100MB
su -c "./fam_create_region myRegion  100 460" user1 || test_failed "Create Region failed: $?" 1
...
su -c "./fam_allocate_data myRegion myData 50 666" user2 2>&1 1>/dev/null 2>/dev/null || test_failed "Allocate DataItem user2 failed: $?"

#Testcases which are failing. Need to be addressed.
# As user2 : Change Region permission
#su -c "./fam_changepermission_region myRegion 666" user2 2>&1 1>/dev/null 2>/dev/null || test_failed "Change Region permission user2 failed: $?"
...
#su -c "./fam_allocate_data myRegion myData 50 666" user3 2>&1 1>/dev/null 2>/dev/null || test_failed "Allocate DataItem user3 failed: $?"

# As user3 : Change Region permission
#su -c "./fam_changepermission_region myRegion 666" user3 2>&1 1>/dev/null 2>/dev/null || test_failed "Change Region permission user3 failed: $?"
...
! su -c "./fam_changepermission_region myRegion 666" user4 2>&1 1>/dev/null 2>/dev/null || test_failed "Change Region permission user4 expected to fail: $?"
