...
echo "Tests DataItem APIs with 440 permission"

#Perm		uid	gid	operuid	opergid	CR	DR	ADi	DDi	Backup	Restore	DeleteBackup
#"R-R---"       1001	1001	1001	1001	-	NA	-	✔	✔	✔	✔
#				1002	1001	-	NA	-	✖	✔	✖	✖
#				1003	1001	-	NA	-	✖	✔	✖	✖
#				1004	1004	-	NA	-	✖	✖	✖	✖
...
su -c "./fam_allocate_data myRegion myData 50 440" user1 || test_failed "Allocate DataItem user1 failed: $?"

# As user1 : Backup DataItem
su -c "./fam_backup_data myRegion myData myBackup" user1 2>&1 1>/dev/null || test_failed "Backup DataItem user1 failed: $?"

# As user1 : Restore to DataItem
su -c "./fam_restore_backup_data myRegion myData myBackup" user1 || test_failed "Restore DataItem user1 failed: $?"

# As user1 : Delete backup
su -c "./fam_delete_backup_data myBackup" user1 || test_failed "Delete myBackup user1 failed: $?"

# As user2 : Backup DataItem
su -c "./fam_backup_data myRegion myData myBackup" user2 2>&1 1>/dev/null || test_failed "Backup DataItem user2 failed: $?"

# As user2 : Restore to DataItem
! su -c "./fam_restore_backup_data myRegion myData myBackup" user2 || test_failed "Restore DataItem user2 expected to fail : $?"

# As user2 : Delete backup
! su -c "./fam_delete_backup_data myBackup" user2 || test_failed "Delete myBackup user2 expected to fail: $?"


# As user3 : Backup DataItem
su -c "./fam_backup_data myRegion myData myBackup" user3 2>&1 1>/dev/null || test_failed "Backup DataItem user3 failed: $?"
...
! su -c "./fam_restore_backup_data myRegion myData myBackup" user4 2>&1 1>/dev/null || test_failed "Restore DataItem user4 expected to fail: $?"

# As user4 : Delete backup
! su -c "./fam_delete_backup_data myBackup" user4 2>&1 1>/dev/null || test_failed "Delete myBackup user4 expected to fail: $?"
