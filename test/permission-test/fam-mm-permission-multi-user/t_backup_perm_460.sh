...
su -c "./fam_backup_data myRegion myData myBackup" user1 || test_failed "Backup DataItem user1 failed: $?"
...
su -c "./fam_backup_data myRegion myData myBackup" user2 || test_failed "Backup DataItem user2 failed: $?"
...
#user3 has user1 as secondary group.Will its permissions be considered as others?
...
#! su -c "./fam_restore_backup_data myRegion myData myBackup" user3 2>&1 1>/dev/null || test_failed "Restore DataItem user3 failed: $?"
...
! su -c "./fam_delete_backup_data myBackup" user4 2>&1 1>/dev/null || test_failed "Delete myBackup user2 expected to fail: $?"
