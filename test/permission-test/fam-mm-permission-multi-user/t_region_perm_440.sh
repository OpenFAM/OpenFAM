...
su -c "./fam_create_region myRegion  100 440" user1 || test_failed "Create Region failed: $?" 1
...
su -c "./fam_lookup_region myRegion" user2 2>&1 1>/dev/null 2>/dev/null || test_failed "Lookup Region user2 expected to succeed: $?"
...
! su -c "./fam_changepermission_region myRegion 460" user2 2>&1 1>/dev/null 2>/dev/null || test_failed "Change Region permission user2 expected to fail: $?"
...
#Commenting Test cases which are failing. Need to be addressed
# As user3:user1 as secondary group perform lookup region on "myRegion"
#su -c "./fam_lookup_region myRegion" user3 2>&1 1>/dev/null 2>/dev/null || test_failed "Lookup Region user3 expected to succeed: $?"
...
#! su -c "./fam_changepermission_region myRegion 460" user3 2>&1 1>/dev/null 2>/dev/null || test_failed "Change Region permission user3 expected to fail: $?"
...
! su -c "./fam_changepermission_region myRegion 460" user4 2>&1 1>/dev/null 2>/dev/null || test_failed "Change Region permission user4 expected to fail: $?"
