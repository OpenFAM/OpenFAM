...
int main(int argc, char **argv)
{
  ...
  cout <<"MinLocalTableSize: " <<  MinLocalTableSize << "W  TableSize: " << TableSize << "W  NumProcs: " << NumProcs << " LocalTableSize: " << LocalTableSize << "W   OneWord(sizeof(u64Int)): " << sizeof(u64Int) << "B\n";
  
  if(atoi(argv[1]) == 0){	
         auto start = high_resolution_clock::now(); 
	     region = GUPS_fam_initialize((u64Int)(TotalMem * 1.5   * sizeof(u64Int)));// Initialize the FAM region
	     try {		
	          Fam_Descriptor *gupsTable = my_fam->fam_allocate(GUPSTable,(u64Int)(TotalMem * sizeof(u64Int)), 0777, region); //Allocate GUPSTable in the FAM region
			  Fam_Descriptor *sAbort = my_fam->fam_allocate(sAbortVariable,	sizeof(int), 0777, region);
			  Fam_Descriptor *rAbort = my_fam->fam_allocate(rAbortVariable,	sizeof(int), 0777, region);
	
			  Fam_Descriptor *GUPs = my_fam->fam_allocate(GUPsVariable,	sizeof(double), 0777, region);
			  Fam_Descriptor *temp_GUPs = my_fam->fam_allocate(temp_GUPsVariable, sizeof(double), 0777, region);
			  Fam_Descriptor *GlbNumErrors = my_fam->fam_allocate(GlbNumErrorsVariable, sizeof(s64Int), 0777, region);
			  Fam_Descriptor *NumErrors = my_fam->fam_allocate(NumErrorsVariable, sizeof(s64Int), 0777, region);
			  
			  Fam_Descriptor *HPCC_Table = my_fam->fam_allocate(HPCC_TableVariable,LocalTableSize*sizeof(u64Int), 0777, region);
			  Fam_Descriptor *count = my_fam->fam_allocate(countVariable, sizeof(s64Int), 0777, region);	
			  Fam_Descriptor *ran = my_fam->fam_allocate(ranVariable, sizeof(s64Int), 0777, region);	 
			  Fam_Descriptor *updates = my_fam->fam_allocate(updatesVariable, sizeof(s64Int) * NumProcs, 0777, region);	
			  Fam_Descriptor *all_updates = my_fam->fam_allocate(all_updatesVariable, sizeof(s64Int), 0777, region);	 /*: An array to collect sum*/
	    ...
	    region = GUPS_fam_initialize(); // Look up the region in the metadata service.
		...
		s64Int *updates ;
		s64Int *all_updates ;
		// Map all defined data items in the FAM to the process virtual address space.
		...
		*sAbort = 0;
		...
		for (i=0; i<LocalTableSize; i++){
			if (operation_mode==0){
				hPCC_Table[i+sizeof(u64Int)*LocalTableSize*MyProc] = MyProc;
			}
			else{
					PE_Id=MyProc;
					my_fam->fam_put_nonblocking(&PE_Id, HPCC_Table, i+sizeof(u64Int)*LocalTableSize*MyProc , sizeof(u64Int)); //  Copy the data element from local memory to FAM.				
					my_fam->fam_barrier_all();
			}
		}
		...
		if(verify){			
			for (i=0; i<LocalTableSize; i++)
				if (operation_mode==0){
					cout << "PEID_"<< MyProc <<": "<<hPCC_Table[i+sizeof(u64Int)*LocalTableSize*MyProc]<<"\n";
				}
				else{
					my_fam->fam_get_nonblocking(&PE_Id, HPCC_Table, i+sizeof(u64Int)*LocalTableSize*MyProc , sizeof(u64Int));
		...
		*ran = starts(4*GlobalStartMyProc);
		...
		for (iterate = 0; iterate < niterate; iterate++){
			*ran = (*ran << 1) ^ ((s64Int) *ran < ZERO64B ? POLY : ZERO64B);
			...
			int offset = sizeof(u64Int) * LocalTableSize * remote_proc;// Determine the corresponding offset for the PE
			int wordIndex = *ran & (LocalTableSize-1); // Determine the word index
			
			
			if (operation_mode==0)
					remote_val = (u64Int) &hPCC_Table[offset + wordIndex];// Load the word from the remote memory
				else{
					my_fam->fam_get_nonblocking(&remote_val, HPCC_Table, offset + wordIndex , sizeof(u64Int));	 // Copy the memory element from FAM to the temporary variable.
					my_fam->fam_barrier_all();
				}
			remote_val ^= *ran;// Modify the word via XORing with a random value
		
			if (operation_mode==0){
				hPCC_Table[offset + wordIndex]	= remote_val;// Store the modified version of remote_val into the remote memory
				}
			else{
				my_fam->fam_put_nonblocking(&remote_val, HPCC_Table, offset + wordIndex , sizeof(u64Int)); //  Copy the data element from local memory to FAM.
				my_fam->fam_barrier_all();
			}

			if(verify){
				updates[thisPeId]=updates[thisPeId]+1; // Update the variable to reflect the number of updates by the corresponding PE
				}
				my_fam->fam_barrier_all();
		...
		if(verify){
			all_updates[0]+=updates[MyProc];// Each PE requires to add its #updates to the global variable all_updates
			my_fam->fam_barrier_all();
			...
			if(MyProc == 0){
			  ...
			  if(ProcNumUpdates*NumProcs == all_updates[0]){ // Check the number of updates by PEs with the expected number of updates
