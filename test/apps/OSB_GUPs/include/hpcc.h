/* -*- mode: C; tab-width: 2; indent-tabs-mode: nil; fill-column: 79; coding:
 * iso-latin-1-unix -*- */
...
/* Define 64-bit types and corresponding format strings for printf() and
 * constants */
...
typedef struct {
    ...
    double SHMEMGUPs, StarGUPs, SingleGUPs, SHMEMRandomAccess_ErrorsFraction,
        SHMEMRandomAccess_time, SHMEMRandomAccess_CheckTime,
        SHMEMRandomAccess_TimeBound, StarStreamCopyGBs, StarStreamScaleGBs,
        StarStreamAddGBs, StarStreamTriadGBs, SingleStreamCopyGBs,
        SingleStreamScaleGBs, SingleStreamAddGBs, SingleStreamTriadGBs,
        StarDGEMMGflops, SingleDGEMMGflops;
    ...
    double MaxPingPongLatency, RandomlyOrderedRingLatency, MinPingPongBandwidth,
        ...
        MinPingPongLatency, AvgPingPongLatency, MaxPingPongBandwidth,
        AvgPingPongBandwidth, NaturallyOrderedRingLatency;
    int DGEMM_N;
    ...
    double SHMEMFFTtimingsForward[SHMEMFFT_TIMING_COUNT],
        SHMEMFFTtimingsBackward[SHMEMFFT_TIMING_COUNT];
    ...
    int RunHPL, RunStarDGEMM, RunSingleDGEMM, RunPTRANS, RunStarStream,
        ...
        RunSingleRandomAccess, RunLatencyBandwidth, RunStarFFT, RunSingleFFT,
        RunSHMEMFFT;

    int FFTEnblk, FFTEnp, FFTEl2size;
    s64Int RandomAccess_N, SHMEMRandomAccess_ExeUpdates, SHMEMRandomAccess_N,
        SHMEMRandomAccess_Errors, SHMEMFFT_N;
} HPCC_Params;
/*
...
-  For tests that have "Star" and "Single" variants (DGEMM, RandomAccess,
STREAM) the function that performs the test returns a value (0 or 1) that
indicates runtime failure and also returns benchamark failure (due to wrong
optimization that causes numerical error) by setting params->Failure.
*/

extern int HPCC_Init(HPCC_Params *params);
extern int HPCC_Finalize(HPCC_Params *params);
extern int HPCC_LocalVectorSize(HPCC_Params *params, int vecCnt, size_t size,
                                int pow2);
...
extern int HPCC_TestFFT(HPCC_Params *params, int doIO, double *UGflops, int *Un,
                        int *Ufailure);
extern int HPCC_TestDGEMM(HPCC_Params *params, int doIO, double *UGflops,
                          int *Un, int *Ufailure);
extern int MaxMem(int nprocs, int imrow, int imcol, int nmat, int *mval,
                  int *nval, int nbmat, int *mbval, int *nbval, int ngrids,
                  int *npval, int *nqval, long *maxMem);
extern int HPCC_RandomAccess(HPCC_Params *params, int doIO, double *GUPs,
                             int *failure);
extern int HPCC_Stream(HPCC_Params *params, int doIO, double *copyGBs,
                       double *scaleGBs, double *addGBs, double *triadGBs,
                       int *failure);
extern void main_bench_lat_bw(HPCC_Params *params);

extern int pdtrans(char *trans, int *m, int *n, int *mb, int *nb, double *a,
                   int *lda, double *beta, double *c__, int *ldc, int *imrow,
                   int *imcol, double *work, int *iwork);
extern int pdmatgen(int *ictxt, char *aform, char *diag, int *m, int *n,
                    int *mb, int *nb, double *a, int *lda, int *iarow,
                    int *iacol, int *iseed, int *iroff, int *irnum, int *icoff,
                    int *icnum, int *myrow, int *mycol, int *nprow, int *npcol,
                    double alpha);
extern int pdmatcmp(int *ictxt, int *m_, int *n_, double *a, int *lda_,
                    double *aCopy, int *ldc_, double *error);
extern int pxerbla(int *ictxt, char *srname, int *info);
extern int slcombine_(int *ictxt, char *scope, char *op, char *timetype, int *n,
...
#define DPRN(i, v)                                                             \
    do {                                                                       \
        printf(__FILE__ "(%d)@%d:" #v "=%g\n", __LINE__, i, (double)(v));      \
        fflush(stdout);                                                        \
    } while (0)

#define BEGIN_IO(r, fn, f)                                                     \
    if (0 == r) {                                                              \
        f = fopen(fn, "a");                                                    \
        if (!f)                                                                \
    fprintf(f = stderr, "Problem with appending to file '%s'\n", fn)
#define END_IO(r, f)                                                           \
    fflush(f);                                                                 \
